...
- **業務エラーの中身は `errors` 配列にしか無い。** gkill は異常時に 4xx/5xx を返すが（ADR-0045）、`error_code` は本文にしか入っていない。**ステータスで打ち切ると `hasAuthError` に到達せずセッションの取り直しが走らなくなり、`IsAlreadyExistTag`（既存タグを成功扱い）も効かなくなる**。`postRaw` は非2xxでも本文が gkill 形式（`errors`/`messages` を持つ JSON）ならそのまま返す形になっている。成功時 `errors` は `[]` ではなく **`null`**
- **403 はローカル限定アクセスとは限らない。** gkill は認可の拒否全般（管理者権限なし・アカウント無効）にも 403 を使う。ローカル限定アクセスの拒否は `ERR000414` で見分ける（本文が読めない 403 のときだけステータスで決め打ちしてよい）
...
- **`/files/` のリトライ条件は 403 と 401 の両方を見る**（`files.go`）。ここはステータスで判定している数少ない場所で、片方だけ見ているとセッションの取り直しが黙って走らなくなる。「リポジトリの取得に失敗した」は 500 なので再試行しない
