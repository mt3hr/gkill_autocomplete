...
const (
	...
	// このツールは確認画面を何日も上げたままにする。
	//
	// **長い期限は gkill の DB に長寿命の鍵を置くということ。**
	// 終了時には必ず消す(Close)が、強制終了されると期限まで残る。
	// 残っても危険が増えないのは、この鍵で入れる先が
	// 「同じ端末の設定ディレクトリに書ける人」の記録だけであるため。
	sessionTTL = 7 * 24 * time.Hour

	// sessionRenewBefore は期限の何分前に発行し直すか。
	//
	// 解析の1往復が終わる前に切れないだけの余裕を取る。
	// 期限が長くなっても、切れる寸前に跨がないための余裕は要る。
	sessionRenewBefore = 1 * time.Hour
)
