...
const (
	...
	// TierUnjudgeable は判定しようがなかった記録。
	//
	// 視覚モデルが復号できない画像形式(RAW・webpなど)や、gkillが画像として
	// 返さなかったもの(動画・書庫・書類)。何度やり直しても結果は変わらないので、
	// 失敗ではなく確定した答えとして評価済みにする。
	TierUnjudgeable = "unjudgeable"
)
